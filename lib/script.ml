...
let script_num_of_bytes (data : Cstruct.t) : int64 =
  let len = Cstruct.length data in
  if len = 0 then 0L
  else if len > 4 then
    (* Numbers in script are limited to 4 bytes *)
...
and exec_opcode_inner (st : eval_state) (op : opcode) (script_code : Cstruct.t)
  ...
  | OP_PICK ->
    ...
    | Error e -> Error e
    | Ok n_bytes ->
      let n = Int64.to_int (script_num_of_bytes n_bytes) in
  ...
  | OP_TUCK ->
    begin match st.stack with
    | a :: b :: rest -> st.stack <- a :: b :: a :: rest; Ok ()
    | _ -> Error "Stack underflow"
    end
  ...
  | OP_2DUP ->
    begin match st.stack with
    | a :: b :: _ -> st.stack <- a :: b :: st.stack; Ok ()
    | _ -> Error "Stack underflow"
    end

  | OP_3DUP ->
    begin match st.stack with
    | a :: b :: c :: _ -> st.stack <- a :: b :: c :: st.stack; Ok ()
    | _ -> Error "Stack underflow"
    end

  | OP_2OVER ->
    begin match st.stack with
    | _ :: _ :: c :: d :: _ -> st.stack <- c :: d :: st.stack; Ok ()
    | _ -> Error "Stack underflow"
    end
  ...
  | OP_1ADD ->
    ...
    | Error e -> Error e
    | Ok a ->
      let n = script_num_of_bytes a in
  ...
  | OP_ADD ->
    ...
    | Ok a ->
      ...
      | Error e -> Error e
      | Ok b ->
        let na = script_num_of_bytes a in
        let nb = script_num_of_bytes b in
  ...
  | OP_WITHIN ->
    ...
    | Ok max_bytes ->
      ...
      | Ok min_bytes ->
        ...
        | Error e -> Error e
        | Ok x_bytes ->
          let x = script_num_of_bytes x_bytes in
          let min_val = script_num_of_bytes min_bytes in
          let max_val = script_num_of_bytes max_bytes in
  ...
  | OP_CODESEPARATOR ->
    (* Just update the codesep position; script_code handling is done elsewhere *)
    Ok ()
    ...
    | Ok pubkey ->
      ...
      | Ok sig_bytes ->
        if Cstruct.length sig_bytes = 0 then
          ...
          (* Compute sighash based on version *)
          let sighash = match st.sig_version with
            | SigVersionBase ->
              (* For legacy, apply FindAndDelete *)
              let cleaned = find_and_delete script_code sig_bytes in
              compute_sighash_legacy st.tx st.input_index cleaned hash_type
            | SigVersionWitnessV0 ->
              compute_sighash_segwit st.tx st.input_index script_code st.amount hash_type
            | SigVersionTaproot | SigVersionTapscript ->
              (* Taproot uses different sighash - simplified here *)
              compute_sighash_segwit st.tx st.input_index script_code st.amount hash_type
          in
    ...
    if st.flags land script_verify_checklocktimeverify = 0 then
      ...
      | Ok lock_bytes ->
        if Cstruct.length lock_bytes > 5 then
          ...
          let locktime = script_num_of_bytes lock_bytes in
          if locktime < 0L then
    ...
    if st.flags land script_verify_checksequenceverify = 0 then
      ...
      | Ok seq_bytes ->
        if Cstruct.length seq_bytes > 5 then
          ...
          let sequence = script_num_of_bytes seq_bytes in
          if sequence < 0L then
  ...
  | OP_CHECKSIGADD ->
    (* Tapscript: pop n, sig, pubkey; push n+1 if valid, n if empty sig, fail otherwise *)
    begin match stack_pop st with
    | Error e -> Error e
    | Ok pubkey ->
      begin match stack_pop st with
...
(* Execute a script and return (success, stack) *)
let eval_script (st : eval_state) (script : Cstruct.t) : (bool, string) result =
...
let verify_script ~(tx : Types.transaction) ~(input_index : int)
  ...
  | P2SH_script hash ->
    ...
    if flags land script_verify_p2sh = 0 then
      ...
      | Error e -> Error e
      | Ok false -> Ok false
      | Ok true -> run_script st script_pubkey
      end
    else begin
      (* Run scriptSig *)
      let st = create_eval_state ~tx ~input_index ~amount ~flags
                ...
                if flags land script_verify_witness = 0 then
                  ...
                  if List.length wit_items <> 2 then
                    ...
                    if not (Cstruct.equal pubkey_hash program) then
                      ...
                      run_script st2 implicit_script
                    end
                  end
                end
        ...
        if List.length wit_items = 0 then
          ...
          if not (Cstruct.equal script_hash program) then
            ...
            let st = create_eval_state ~tx ~input_index ~amount ~flags
                       ~sig_version:SigVersionWitnessV0 in
            st.stack <- wit_stack;
            run_script st witness_script
          end
        end
      end
    end

  | P2TR_script _program ->
    (* Taproot (BIP-341/342) - simplified, full implementation needs Schnorr *)
    if flags land script_verify_taproot = 0 then
      ...
      if Cstruct.length script_sig <> 0 then
        ...
        | [sig_bytes] ->
          (* Key path spend: single 64-byte Schnorr signature *)
          if Cstruct.length sig_bytes <> 64 && Cstruct.length sig_bytes <> 65 then
            Error "Invalid taproot signature length"
          else begin
            (* Simplified: would need Schnorr verification *)
            (* For now, return true as placeholder *)
            Ok true
          end
        | _ ->
          (* Script path spend: more complex taproot script execution *)
          (* Simplified implementation *)
          let annex_present =
            List.length wit_items >= 2 &&
            let last = List.nth wit_items (List.length wit_items - 1) in
            Cstruct.length last > 0 && Cstruct.get_uint8 last 0 = 0x50
          in
          let wit_items = if annex_present then
            List.filteri (fun i _ -> i < List.length wit_items - 1) wit_items
          else wit_items in
          if List.length wit_items < 2 then
            Error "Taproot script path requires at least 2 items"
          else begin
            let _control = List.nth wit_items (List.length wit_items - 1) in
            let tap_script = List.nth wit_items (List.length wit_items - 2) in
            let wit_stack = List.rev (List.filteri (fun i _ -> i < List.length wit_items - 2) wit_items) in
            (* Would need to verify control block and merkle path *)
            let st = create_eval_state ~tx ~input_index ~amount ~flags
                       ~sig_version:SigVersionTapscript in
            st.stack <- wit_stack;
            run_script st tap_script
          end
      end
    end
