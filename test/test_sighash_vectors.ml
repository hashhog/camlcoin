...
(* Convert Cstruct to lowercase hex string *)
...
let test_sighash_vectors () =
  ...
  List.iteri (fun i (raw_tx, script_hex, input_index, hash_type, expected_hex) ->
    try
      ...
      if String.lowercase_ascii got_hex = String.lowercase_ascii expected_hex then
        incr passed
      else begin
        incr failed;
        Printf.printf "FAIL vector %d: expected %s, got %s\n" i expected_hex got_hex
      end
    with exn ->
      incr errors;
